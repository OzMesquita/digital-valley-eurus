...
import ufc.russas.encontrosuniversitarios.view.adapter.ActivityEvaluationAdapter;
...
public class ActivityEvaluationActivity extends AppCompatActivity {
    ...
    private List<ActivityCriteria> criterios;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getParcelableExtra(ATIVIDADE) != null) {
            Activity activity = getIntent().getParcelableExtra(ATIVIDADE);
            activityEvaluationViewModel.init(activity);
        }
        ...
        activityEvaluationViewModel = ViewModelProviders.of(this).get(ActivityEvaluationViewModel.class);
        txtActivityName.setText(activityEvaluationViewModel.getActivity().getName());
        ...
        activityEvaluationViewModel.getCriterios().observe(this, new Observer<List<ActivityCriteria>>() {
            @Override
            public void onChanged(List<ActivityCriteria> activityCriteria) {
                if (activityCriteria != null) {
                    criterios = activityCriteria;
                    ActivityEvaluationAdapter adapter = new ActivityEvaluationAdapter(criterios, criteriaListener);
                    criteriosRec.setAdapter(adapter);
                    criteriaListener.onNotaChanged();
                }
            }
        });
        ...
        activityEvaluationViewModel.listarCriterios(new ActivitiesListener() {
            @Override
    ...
    private CriteriaListener criteriaListener = new CriteriaListener() {
        @Override
        public void onNotaChanged() {
            double total = 0;
            for (ActivityCriteria activityCriteria : criterios) {
                Log.i("teste ", ""+ activityCriteria.getNota());
                total += activityCriteria.getNota();
            }
    ...
    public void showConfirmarAvaliacaoDialog() {
        ...
        nomeAtividadeAvaliada.setText(activityEvaluationViewModel.getActivity().getName());
        ...
        builder.setPositiveButton("Sim", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                activityEvaluationViewModel.avaliarAtividade(comentarios.getText().toString(),
                        getBaseContext(), new EvaluationListener() {
                            @Override
